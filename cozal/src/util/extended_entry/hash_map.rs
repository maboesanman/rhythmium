use std::collections::hash_map::{
    HashMap, RandomState, RawEntryMut, RawOccupiedEntryMut, RawVacantEntryMut,
};
    ...
    S: BuildHasher,
{
    ...
    match entry {
        RawEntryMut::Occupied(entry) => Ok(OccupiedExtEntry { hash_map, entry }),
...
impl<'a, K: Hash + Eq, V, S: BuildHasher> VacantExtEntry<'a, K, V, S> {
    ...
    pub fn occupy(self, value: V) -> OccupiedExtEntry<'a, K, V, S> {
        ...
        OccupiedExtEntry { hash_map, entry }
    }
