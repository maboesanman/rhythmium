...
use crate::source::source_poll::{self, TrySourcePoll};
use crate::source::sources::transposer::channels::original_step_future::OriginalStepPoll;
                    ...
                    let free = match original.poll(step, &all_channel_waker) {
                        OriginalStepPoll::OutputEvent(event) => {
                            return Ok(SourcePoll::Interrupt {
                                time: step.get_time(),
                                interrupt: source_poll::Interrupt::FinalizedEvent(event),
                            })
                        }
    ...
    fn poll_events(
        ...
        let poll = loop {
            let (poll, time) = match self
                ...
                .unwrap()
            {
                BeforeStatusEvents::Ready { next_time } => {
                    break SourcePoll::Ready {
                        state: (),
                        next_event_at: next_time,
                    }
                }
                BeforeStatusEvents::Saturating { step, .. } => {
