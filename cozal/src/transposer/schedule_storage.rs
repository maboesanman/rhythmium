...
pub trait StorageFamily: Copy + 'static {
    ...
    type Transposer: SharedPointerKind;

    type LazyState: SharedPointerKind;
}
...
impl StorageFamily for DefaultStorage {
    type OrdMap<K: Ord + Eq + Clone, V: Clone> = rpds::RedBlackTreeMapSync<K, V>;
    type HashMap<K: Hash + Eq + Clone, V: Clone> = rpds::HashTrieMapSync<K, V>;
...
impl<K: Ord + Eq + Clone, V: Clone> OrdMapStorage<K, V> for rpds::RedBlackTreeMapSync<K, V> {
    fn new() -> Self {
        Self::new()
    }
