...
use super::{Transposer, TransposerInput, TransposerInputEventHandler};
...
impl<I: TransposerInput> InputStateMapErased for InputStateMap<I> {
    ...
    fn get_request(&self) -> Option<NonNull<()>> {
        self.requested_input.as_ref().map(|i| NonNull::from(i).cast())
    }
...
impl<T: Transposer> InputStateManager<T> {
    ...
    pub fn provide_input_state<I>(&mut self, input: I, state: NonNull<I::InputState>) -> Result<(), NonNull<I::InputState>>
    where
        I: TransposerInput<Base = T>,
        T: TransposerInputEventHandler<I>,
    {
